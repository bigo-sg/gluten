...
public abstract class ArrowVectorWriter {
    ...
    // Exact class matches
    private static Map<Class<? extends Type>, WriterBuilder> writerBuilders = Map.ofEntries(
        Map.entry(IntegerType.class, (fieldType, allocator, vector) ->
            new IntVectorWriter(fieldType, allocator, vector)),
        Map.entry(BooleanType.class, (fieldType, allocator, vector) ->
            new BooleanVectorWriter(fieldType, allocator, vector)),
        Map.entry(BigIntType.class, (fieldType, allocator, vector) ->
            new BigIntVectorWriter(fieldType, allocator, vector)),
        Map.entry(DoubleType.class, (fieldType, allocator, vector) ->
            new Float8VectorWriter(fieldType, allocator, vector)),
        Map.entry(VarCharType.class, (fieldType, allocator, vector) ->
            new VarCharVectorWriter(fieldType, allocator, vector)),
        Map.entry(TimestampType.class, (fieldType, allocator, vector) ->
            new TimestampVectorWriter(fieldType, allocator, vector)),
        Map.entry(RowType.class, (fieldType, allocator, vector) ->
            new StructVectorWriter(fieldType, allocator, vector)),
        Map.entry(ArrayType.class, (fieldType, allocator, vector) ->
            new ArrayVectorWriter(fieldType, allocator, vector)),
        Map.entry(MapType.class, (fieldType, allocator, vector) ->
            new MapVectorWriter(fieldType, allocator, vector))
    );
...
class FieldVectorCreator {
    ...
    // Exact class matches
    private static Map<Class<? extends Type>, ArrowTypeConverter> arrowTypeConverters = Map.ofEntries(
        ...
        Map.entry(DoubleType.class, (dataType, timeZoneId) -> new ArrowType.FloatingPoint(FloatingPointPrecision.DOUBLE)),
        Map.entry(VarCharType.class, (dataType, timeZoneId) -> ArrowType.Utf8.INSTANCE),
        Map.entry(TimestampType.class, (dataType, timeZoneId) ->
            new ArrowType.Timestamp(TimeUnit.MILLISECOND, timeZoneId == null ? "UTC" : timeZoneId))
    );
    ...
    private static Field toArrowField(
        ...
        if (dataType instanceof ArrayType) {
            ...
            List<String> fieldNames =
                Arrays.asList(
                    MapVector.KEY_NAME, MapVector.VALUE_NAME);
            List<Type> fieldTypes = mapType.getChildren();
            RowType structType = new RowType(fieldNames, fieldTypes);
            Field structField = toArrowField(MapVector.DATA_VECTOR_NAME, structType, nullable, timeZoneId);
...
class ArrayVectorWriter extends BaseVectorWriter<ListVector, ArrayData> {
    ...
    public ArrayVectorWriter(Type fieldType, BufferAllocator allocator, FieldVector vector) {
        ...
        this.elementWriter =
            ArrowVectorWriter.create("element", elementType, allocator, elementVector);
    }
...
class MapVectorWriter extends BaseVectorWriter<MapVector, MapData> {
    ...
    public MapVectorWriter(Type fieldType, BufferAllocator allocator, FieldVector vector) {
        ...
        this.keyWriter =
            ArrowVectorWriter.create(MapVector.KEY_NAME, mapType.getChildren().get(0), allocator, keyVector);
        this.valueWriter =
            ArrowVectorWriter.create(MapVector.VALUE_NAME, mapType.getChildren().get(1), allocator, valueVector);
    }
