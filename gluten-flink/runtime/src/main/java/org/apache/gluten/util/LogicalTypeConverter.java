...
public class LogicalTypeConverter {
    ...
    public static Type toVLType(LogicalType logicalType) {
        if (logicalType instanceof RowType) {
            ...
            List<Type> fieldTypes = flinkRowType.getChildren().stream().map(LogicalTypeConverter::toVLType).collect(Collectors.toList());
            return new io.github.zhztheplayer.velox4j.type.RowType(flinkRowType.getFieldNames(), fieldTypes);
