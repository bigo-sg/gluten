...
public class GlutenDelegatePlannerFactory extends BaseDelegateFactory<PlannerFactory> implements PlannerFactory {

  private static final Logger LOG = LoggerFactory.getLogger(GlutenDelegatePlannerFactory.class);

  private final Set<SqlDialect> seenDialect = new HashSet<>();

    public GlutenDelegatePlannerFactory() {
        super(PlannerModule.getInstance().loadPlannerFactory());
        LOG.info("create gluten delegate factory here...");
